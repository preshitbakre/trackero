  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-[22px] font-semibold text-neutral-700 dark:text-dneutral-700">Projects</h1>
        {isAdmin && (
      ...
      {projects.length === 0 ? (
        <div className="text-center py-12 text-neutral-400 dark:text-dneutral-500">
          <p className="text-[22px]">No projects yet</p>
          <p className="text-[16px] mt-1">Create your first project to get started</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <Link
              ...
              className="block p-4 rounded-lg shadow-sm dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)] hover:shadow-md dark:hover:shadow-[0_4px_12px_rgba(0,0,0,0.4)] transition-shadow"
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[16px] font-mono px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-dneutral-200 text-neutral-500 dark:text-dneutral-500">
                  {project.prefix}
                </span>
                <span className={`text-[16px] px-1.5 py-0.5 rounded ${
                  project.status === 'active'
                    ? 'bg-mint-light dark:bg-mint-dm/30 text-neutral-700 dark:text-mint-dm'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-700/50" onClick={onClose}>
      <div className="bg-white dark:bg-dneutral-100 rounded-lg p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && <div className="text-[16px] text-danger dark:text-danger">{error}</div>}
          <div>
