...
function getDueLabel(endDate: string): { text: string; className: string } | null {
  ...
  if (diff <= 7) return { text: `Due in ${diff}d`, className: 'text-neutral-500 bg-neutral-100' };
  return null;
}
...
export function TaskRow({ taskKey, title, priority, status, endDate, hasBlockers, onClick }: TaskRowProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2 w-full text-left px-3 py-2.5 rounded-lg hover:bg-neutral-100 transition-colors group"
    >
      ...
      <span className="text-[16px] font-mono text-neutral-400 flex-shrink-0">{taskKey}</span>
      <span className="text-[16px] text-neutral-700 truncate flex-1 group-hover:text-lilac-dark">{title}</span>
      <span
