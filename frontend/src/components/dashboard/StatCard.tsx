  ...
  return (
    <div className="rounded-xl bg-card p-5">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[11px] font-semibold text-mute uppercase tracking-[0.18em]">{label}</span>
      </div>
      <div className={`font-serif italic text-[40px] leading-none ${valueColor || 'text-text'}`}>
        {value}
      </div>
      {subtext && (
        <p className="text-[12px] text-mute mt-2">{subtext}</p>
      )}
      {progressBar && (
        <div className="mt-3 h-1 rounded-full bg-rule">
          <div
