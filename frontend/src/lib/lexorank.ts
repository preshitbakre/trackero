/**
 * Shared algorithm with backend `backend/src/common/helpers/lexorank.ts`
 * (Task 5.2). Same `a..z` alphabet and identical semantics, so keys produced
 * here are interoperable with the backend and vice-versa. Keep both copies in
 * sync: if you change one, port the change to the other.
 *
